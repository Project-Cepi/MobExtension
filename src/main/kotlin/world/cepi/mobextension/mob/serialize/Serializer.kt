...
fun serialize() {
    ...
    mobFolder.list(filter)?.forEach {
        ...
        try {
            medeia.parseAll(medeia.createJsonReader(getSchema(), file.reader()))
        } catch (e: ValidationFailedException) { MobExtension.logger.error("[MobExtension] $it cannot be loaded due to invalid JSON!") }
    }
}
